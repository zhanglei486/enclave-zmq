...
int zmq::curve_server_t::process_hello (msg_t *msg_)
{
    ...
    uint8_t *helloMsg = (uint8_t *)malloc(size);
    memcpy(helloMsg, hello, size); 
    enclave_ProcessHello(enclave, &result, helloMsg, size);
    free(helloMsg);
    if(result!=OE_OK){
        session->get_socket ()->event_handshake_failed_protocol (
...
int zmq::curve_server_t::produce_welcome (msg_t *msg_)
{
    ...
    oe_enclave_t *enclave = (oe_enclave_t *)_enclave;
    oe_result_t result;
    enclave_ProduceWelcome(enclave, &result, &welcomeMsg, &welcomeMsgLen);
    if( result != OE_OK ){
        return -1;
    }
    memcpy( welcome, welcomeMsg, welcomeMsgLen);
    if(welcomeMsg)
	    free(welcomeMsg);
#else    
...
int zmq::curve_server_t::process_initiate (msg_t *msg_)
{
    ...
    oe_result_t result;
    oe_enclave_t *enclave = (oe_enclave_t *)_enclave;
    uint8_t *metaMsgOut=NULL;
    size_t metaMsgLen;
    uint8_t *initiateMsg=(uint8_t *)malloc(size);
    memcpy(initiateMsg, initiate, size);
    enclave_ProcessInitiate(enclave, &result, initiateMsg, size, &metaMsgOut, &metaMsgLen);
    free(initiateMsg);
    if(result!=OE_OK){
        session->get_socket ()->event_handshake_failed_protocol (
          session->get_endpoint (), ZMQ_PROTOCOL_ERROR_ZMTP_CRYPTOGRAPHIC);
...
int zmq::curve_server_t::produce_ready (msg_t *msg_)
{
...
#ifdef USE_ENCLAVE
    uint8_t *meta_data_ =
      static_cast<uint8_t *> (malloc (metadata_length));
    alloc_assert (meta_data_);
    add_basic_properties (meta_data_, metadata_length);
    ...
    size_t readyMsgLen;
    enclave_ProduceReady(enclave, &result, meta_data_, metadata_length,
    ...
    int rc = msg_->init_size (readyMsgLen);
    errno_assert (rc == 0);
    uint8_t *read = static_cast<uint8_t *> (msg_->data ());
    memcpy(read, readyMsg, readyMsgLen);
    free(readyMsg);
#else    
