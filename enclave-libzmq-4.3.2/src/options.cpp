...
static oe_enclave_t* create_enclave(const char* enclavePath);
    ...
    monitor_event_version (1)
{
    ...
    // enclave
    memset (curve_sealed_secret_key, 0, 1024);
    curve_sealed_key_len=0;
    enclave=NULL;
                                ...
                                size_t optvallen_)
{
    ...
    switch (option_) {
        ...
        case ZMQ_CURVE_SERVER:
	        ...
	        if(enclave==NULL){
                    enclave = create_enclave("./enclave_curve.signed");
	        }
	        {
                    oe_result_t result = OE_OK;
		    if(value==1)
		        enclave_SetCurveZMQ((oe_enclave_t*)enclave, &result, CURVE_SERVER);
		}
                return 0;
            }
            break;
		...
		memcpy(curve_sealed_secret_key, optval_, optvallen_);
		curve_sealed_key_len = optvallen_;
	    }
	    if(enclave==NULL){
                enclave = create_enclave("./enclave_curve.signed");
	    }
	    {
                oe_result_t result = OE_OK;
		enclave_ImportSealedPrivateKey((oe_enclave_t*)enclave, &result,
                     (sealed_data_t *)curve_sealed_secret_key, curve_sealed_key_len, curve_public_key, CURVE_KEYSIZE);
		if( result != OE_OK){
			printf("enclave_ImportSealedPrivateKey error(%d)\n", result);
			break;
		}else{
		     return 0;
		}
	    ...
	    }
	    {
                oe_result_t result = OE_OK;
		//enclave_SetCurveZMQ((oe_enclave_t*)enclave, &result, CURVE_SERVER);
		enclave_ImportServerPublicKey((oe_enclave_t*)enclave, &result,
                          curve_server_key, CURVE_KEYSIZE);
		if( result != OE_OK){
			printf("enclave_ImportServerPublicKey error(%d)\n", result);
			break;
		}else{
		     return 0;
		}
