...
int zmq::curve_mechanism_base_t::encode (msg_t *msg_)
{
    ...
    if (msg_->flags () & msg_t::command)
        flags |= 0x02;
    uint8_t *message_plaintext = static_cast<uint8_t *> (malloc (msg_->size()+1));
    size_t msg_len = msg_->size()+1;
    alloc_assert (message_plaintext);
    message_plaintext[0]=flags;
    memcpy (&message_plaintext[1], msg_->data (), msg_->size());
    ...
    uint8_t *encMsg=NULL;
    size_t encMsgLen;
    enclave_ProduceMessage(enclave, &result, message_plaintext, msg_len,
        &encMsg, &encMsgLen);
    ...
    int rc = msg_->init_size (encMsgLen);
    zmq_assert (rc == 0);

    uint8_t *message = static_cast<uint8_t *> (msg_->data ());
    memcpy(message, encMsg, encMsgLen);
    free(encMsg);
...
int zmq::curve_mechanism_base_t::decode (msg_t *msg_)
{
    ...
    oe_result_t result;
    oe_enclave_t *enclave=(oe_enclave_t *)_enclave;
    uint8_t *encMsg=(uint8_t *)malloc(size);
    memcpy(encMsg, message, size);
    uint8_t *commMsg=NULL;
    size_t commMsgLen;
    enclave_ProcessMessage(enclave, &result, encMsg, size,
        &commMsg, &commMsgLen);
    free(encMsg);
    ...
    rc = msg_->close ();
    rc = msg_->init_size (commMsgLen-1);
    zmq_assert (rc == 0);

    const uint8_t flags = commMsg[0];
    if (flags & 0x01)
        msg_->set_flags (msg_t::more);
    if (flags & 0x02)
        msg_->set_flags (msg_t::command);

    memcpy (msg_->data (), &commMsg[1], commMsgLen-1);
    free(commMsg);
    rc = 0;
#else    
    uint8_t message_nonce[crypto_box_NONCEBYTES];
