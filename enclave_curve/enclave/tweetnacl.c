...
int crypto_box(u8 *c,const u8 *m,u64 d,const u8 *n,const u8 *y,const u8 *x)
{
  ...
  {
    int i;
    printf("crypto_box_beforenm, key:\n");
    for(i=0;i<32;i++){
      printf("%02x", x[i]&0xff);
    }printf("\n");
    for(i=0;i<32;i++){
...
static mbedtls_ctr_drbg_context tnacl_ctr_drbg_contex;
static mbedtls_entropy_context tnacl_entropy_context;

void init_mbedtls_random()
{
    char pers[256];

    mbedtls_entropy_init(&tnacl_entropy_context);
    mbedtls_ctr_drbg_init(&tnacl_ctr_drbg_contex);

    sprintf(pers, "random data string %ld:%d", time(NULL), getpid());
    // mbedtls_ctr_drbg_seed seeds and sets up the CTR_DRBG entropy source for
    // future reseeds.
    mbedtls_ctr_drbg_seed(
        &tnacl_ctr_drbg_contex,
        mbedtls_entropy_func,
        &tnacl_entropy_context,
        (unsigned char*)pers,
        sizeof(pers));
    is_random_init = 1;
}
...
void randombytes (unsigned char *x,unsigned long long xlen)
{
    ...
    mbedtls_ctr_drbg_random(&tnacl_ctr_drbg_contex, x, xlen);  
    return ;
}
